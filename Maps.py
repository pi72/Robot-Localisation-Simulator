...
def Arena_maps(Num_Map):
	...
	if float(Num_Map)==1:
		...
		longest_Dist = math.sqrt((max(Selected_Map_Y)-min(Selected_Map_Y))**2 + (max(Selected_Map_X)-min(Selected_Map_X))**2)
	
	# Map number x can be added here as above
	
	return Selected_Map_X,Selected_Map_Y,Selected_Map_vertex,path,patch,longest_Dist